package com.xiaour.spring.boot.serverImpl;
...
import com.xiaour.spring.boot.exception.PandaLockException;
import com.xiaour.spring.boot.server.DistributedLock;
...
public class ZkPandaLock extends DistributedLock {
	...
	public void lock() {
		...
		try {
			createComPrtitorNode();
			allCompetitorList = pandaZk.getChildren(lockPath, false);
			//allCompetitorList.forEach((String s) -> System.out.println(s));
		} catch (KeeperException e) {
			throw new PandaLockException("zookeeper connect error");
		} catch (InterruptedException e) {
	...
	private void createComPrtitorNode() throws KeeperException, InterruptedException {
		...
		System.out.println(thisCompetitorPath);
	}
	...
	public void connectZooKeeper(String zkhosts, String lockName)
		...
		if (pandaZk == null) {
			pandaZk = new ZooKeeper(zkhosts, DEFAULT_SESSION_TIMEOUT, new Watcher() {
				@Override
				public void process(WatchedEvent event) {
		...
		if (rootStat == null) {
			rootPath = pandaZk.create(ROOT_PANDALOCK_NODE, null, Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
		} else {
		...
		if (lockStat != null) {// 说明此锁已经存在
			...
			// throw new
			// PandaLockException("the lockName is already exits in zookeeper, please check
			// the lockName :"
			// +lockName);
		} else {
			// 创建相对应的锁节点
			lockPath = pandaZk.create(lockNodePathString, null, Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
		}

		this.lockName = lockName;
		System.out.println(this.lockPath+"++++++++++++");
	}
